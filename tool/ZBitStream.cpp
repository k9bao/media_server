...
ZBitStream::ZBitStream()
    : m_pStreamBuffer(NULL), m_nStreamBuffer(0), m_pStreamBeginBuffer(NULL), m_pStreamTailBuffer(NULL), m_nStreamBits(0), m_cDataByte(0), m_nDataBits(0) {
}
...
UINT32 ZBitStream::GetBits(UINT nBits) {
    ...
    if (nBits <= 32) {
        ...
        ASSERT((m_pStreamBuffer <= m_pStreamTailBuffer));
        ASSERT((m_nStreamBuffer >= 0));
